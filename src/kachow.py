...
class CircleOfExpansionEstimator:
    ...
    def find_optical_flow_new_points(self, gray):
        ...
        if self.display:
            ...
            for i, (new, old) in enumerate(zip(good_new, good_old)): # plot keypoints and optical flow vectors
                ...
                # slope = (b - d) / (a - c)
                # b = b - slope * a
                # y_left = int(slope * 0 + b)
                # y_right = int(slope * self.display_img.shape[1] + b)
                # cv2.line(self.mask, (0, y_left), (self.display_img.shape[1], y_right), (0, 255, 0), 2)
                a, b, c, d = int(a), int(b), int(c), int(d)

                self.mask = cv2.line(self.mask, (a, b), (c, d), (0, 255, 0), 2)
                self.display_img = cv2.circle(self.display_img, (a, b), 2, (0, 0, 255), -1)
    ...
    def find_features_to_track(self, gray, good_new_features):
        if good_new_features is None or self.tracking_counter % self.frames_to_track == 0:
            ...
            self.current_features = good_new_features.reshape(-1, 1, 2) # TODO check this
                
        
    def find_circle_from_optical_flow(self, good_new_features, starting_points):
        # use least squares
        vectors = good_new_features - starting_points
        A = np.vstack([vectors[:, 0], vectors[:, 1]]).T
        
        if self.display:
            cv2.circle(self.display_img, (int(center[0]), int(center[1])), 5, (0, 255, 0), -1)
